"""Per-cobra black-dot depths, read from the calibration file.
...
DOT_TARGET_FILE = os.path.join(os.path.dirname(__file__), '..', 'etc',
                               'cobra_dot_target.csv')
...
def loadDotTargets(nCobras, path=None, cmd=None):
    ...
    path : `str`, optional
        Calibration file; defaults to the one shipped with the product.
    cmd : optional
    ...
    path = DOT_TARGET_FILE if path is None else path
